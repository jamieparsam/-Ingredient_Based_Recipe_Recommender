...
data_gen.py
===========
Synthetic dataset generator for the Ingredient-Based Recipe Recommendation System.

-------------------
We used synthetic data because while there are great datasets out there that
organize information into food type, vegetarian/non-vegetarian, and a
description of the food, it is not tailored to college students. Short prep
times, budget-friendly and common ingredients found in a college student's dorm
is necessary to know in order to have an accurate system. We generated a
sythetic dataset that represents dietary needs and shows a realistic college pantry
that many students would have such as eggs, rice, pasta, and canned goods. This
dataset will also produce a clean, reproducible baseline for model evaluation.

The Kaggle CSVs (food_recipes.csv, 1662574418893344.csv) are loaded and merged
inside etl_pipeline.py to augment this synthetic base.

Output
------
  data/synthetic_recipes.csv  — 200+ mock recipes ready for the ETL pipeline.
"""
...
def _pick_ingredients(category: str, n_main: int = None) -> list[str]:
    """
    Build a realistic ingredient list for the given dietary category.
      - All recipes share a common vegan base or pantry staples.
      - Vegetarian recipes add dairy/egg extras.
      - Non-vegetarian recipes add meat/seafood extras.
    """
...
def generate_recipes(n_per_category: int = 70) -> list[dict]:
    ...
    We use predefined name/description pools and cycle through them with slight
    variation (randomised prep time within ±5 min) to produce variety while
    keeping names meaningful and human-readable.
    """
    records = []
    recipe_id = 1
    ...
    for category, pool in category_map.items():
        for i in range(n_per_category):
            # Cycle through the name pool; suffix with index to avoid duplicates
            base_name, base_time, base_desc = pool[i % len(pool)]
            suffix = f" #{i // len(pool) + 1}" if i >= len(pool) else ""
            name = base_name + suffix

            # Add small random jitter so prep times are not all identical
...
if __name__ == "__main__":
    recipes = generate_recipes(n_per_category=70)   # → 210 total records
    save_to_csv(recipes)
